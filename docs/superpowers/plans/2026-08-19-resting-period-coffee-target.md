# Resting Period: Coffee Name + Custom Rest Target — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add two optional URL-persisted inputs to the existing `/resting-period` page — coffee name (display only) and a custom rest-day target (extra card above fixed milestones).
...
**Spec:** `docs/superpowers/specs/2026-08-18-resting-period-page-design.md` (updated 2026-08-19)

**Worktree:** `/Users/muhammadilham/Documents/GitHub/agroastery-web/.worktrees/resting-period`, branch `feature/resting-period-page` (feature already implemented through commit 2d1854c; this plan extends it).
...
**Files:**
- Modify: `components/resting-period/date.ts`
- Test: `components/resting-period/date.test.ts` (append)

- [ ] **Step 1: Write the failing tests**

Append to `components/resting-period/date.test.ts` (inside existing imports, add `parseCoffeeName`, `parseRestDays` to the import from `./date`):
...
Append to `components/resting-period/date.ts`:
...
git commit -m "feat(resting-period): add coffee name and rest days param parsers"
```
...
**Files:**
- Modify: `components/resting-period/index.tsx`
- Test: `components/resting-period/index.test.tsx` (append)

- [ ] **Step 1: Write the failing tests**

Append to `components/resting-period/index.test.tsx`. Update the `vi.mock("next/navigation")` factory's `useSearchParams` to build params from a mock object instead of only `roast` — replace the hoisted block and mock factory at the top of the file:
...
Append these tests inside the existing `describe("RestingPeriod", ...)`:
...
In `components/resting-period/index.tsx`:
...
git commit -m "feat(resting-period): add coffee name and custom rest target inputs"
```
...
Run: `npx eslint components/resting-period/`
Expected: No issues.

- [ ] **Step 2: Build**

Run: `pnpm build`
Expected: Succeeds, `/resting-period` still static (○).

- [ ] **Step 3: Commit any fixes**

Only if Steps 1-2 required changes:
```bash
git add -A && git commit -m "fix(resting-period): address lint/build findings"
```
