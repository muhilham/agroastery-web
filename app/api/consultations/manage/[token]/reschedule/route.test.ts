...
describe("POST reschedule", () => {
  ...
  it("404s for unknown token", async () => {
    ...
    const res = await POST(req({ booking_date: "2026-07-28", time_slot: "11:00" }), ctx("bad"));
    expect(res.status).toBe(404);
  });

  it("400s when booking not confirmed", async () => {
    mockSingle.mockResolvedValue({ data: { id: "b", status: "cancelled", booking_date: "2026-07-28" }, error: null });
    const res = await POST(req({ booking_date: "2026-07-28", time_slot: "11:00" }), ctx("t"));
    expect(res.status).toBe(400);
  });

  it("409s with SLOT_TAKEN on unique violation", async () => {
    mockSingle.mockResolvedValue({
      data: { id: "b", status: "confirmed", booking_date: "2026-07-28", email: "b@e.com", name: "B", time_slot: "11:00" },
      error: null,
    });
    mockUpdateEq.mockResolvedValue({ error: { code: "23505", message: "dup" } });
    const res = await POST(req({ booking_date: "2026-07-29", time_slot: "14:00" }), ctx("t"));
  ...
  it("reschedules a confirmed booking", async () => {
    mockSingle.mockResolvedValue({
      data: { id: "b", status: "confirmed", booking_date: "2026-07-28", email: "b@e.com", name: "B", time_slot: "11:00", manage_token: "t" },
      error: null,
    });
